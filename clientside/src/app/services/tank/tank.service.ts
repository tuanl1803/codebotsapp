...
import {TankModelAudit} from '../../models/tank/tank.model.state';
...
/**
 * Tank service used to handle all CRUD operations against this entity.
 */
@Injectable({
	providedIn: 'root'
})
export class TankService extends AbstractHttpService<TankModel, TankModelAudit> {
	...
	protected readonly classFragments = {
		properties: gql`
			fragment TankProperties on Tank {
				...TankBaseProperties
				name
				width
				length
	 ...
	 * @inheritDoc
	 */
	count(): Observable<PassableStateConfig<TankModel>> {
	 ...
	 * @inheritDoc
	 */
	getAll(): Observable<PassableStateConfig<TankModel>> {
	 ...
	 * @inheritDoc
	 */
	get(id: string, queryParams?: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.query<any>(
			...
			map(result => {
				...
				return {
					targetModelId: id,
					targetModel: result.data.TankById as TankModel,
	 ...
	 * @inheritDoc
	 */
	getWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.query<any>(
			{
				query: gql`
					query GetWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!], $where: [[Where!]!]) {
						tanks: tanks(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy, where: $where) {
							...TankProperties
							${this.getExpands(queryParams.expands)}
						}

						totalCount: countTanks(where: $where)
			...
			map(result => {
				...
				return {
					...
					targetModels: result.data.tanks as TankModel[],
	 ...
	 * @inheritDoc
	 */
	getLastWithQuery(queryParams: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.query<any>(
			{
				query: gql`
					query GetLastWithQuery($pageIndex: Int, $pageSize: Int, $orderBy: [OrderBy!]!) {
						tanks: lastTanks(pageIndex: $pageIndex, pageSize: $pageSize, orderBy: $orderBy) {
							...TankProperties
						}
	 ...
	 * @inheritDoc
	 */
	create(entity: TankModel, queryParams?: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Create($tank: TankInput!) {
			...
			tap(() => this.toastrService.success('Created Tank entity successfully')),
			map(result => {
				...
				return {
					targetModel: result.data.tank as TankModel,
	 ...
	 * @inheritDoc
	 */
	createAll(entities: TankModel[], queryParams?: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation createAll($tanks: [TankInput!]!) {
						tank: createAllTank(tanks: $tanks) {
							...TankProperties,
							${this.getExpands(queryParams ? queryParams.expands: [])}
						}
				...
				`,
				variables: {
					tanks: entities.map(entity => entity.toJSON()),
			...
			tap(() => this.toastrService.success('Created Multiple Tank entities successfully')),
			map(result => {
				...
				return {
					targetModels: result.data.tank as TankModel[],
	 ...
	 * @inheritDoc
	 */
	update(entity: TankModel, queryParams?: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Update($tank: TankInput!) {
			...
			tap(() => this.toastrService.success('Updated Tank entity successfully')),
			map(result => {
	 ...
	 * @inheritDoc
	 */
	updateAll(entities: TankModel[], queryParams?: QueryParams): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation updateAll($tanks: [TankInput!]!) {
						tank: updateAllTank(tanks: $tanks) {
							...TankProperties,
							${this.getExpands(queryParams ? queryParams.expands: [])}
						}
			...
			tap(() => this.toastrService.success('Update Multiple Tank entities successfully')),
			map(result => {
	 ...
	 * @inheritDoc
	 */
	delete(id: string): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation Delete($tankId: ID!) {
						tankId: deleteTankById(tankId: $tankId)
			...
			tap(() => this.toastrService.success('Deleted Tank entity successfully')),
			map(result => {
				...
				return {
					targetModelId: result.data.tankId as string,
	 ...
	 * @inheritDoc
	 */
	deleteAll(ids: string[]): Observable<PassableStateConfig<TankModel>> {
		...
		return this.apollo.mutate<any>(
			{
				mutation: gql`
					mutation DeleteAll($tankIds: [ID!]!) {
						tankIds: deleteAllTankByIds(tankIds: $tankIds)
			...
			tap(() => this.toastrService.success('Deleted all Tank entities successfully')),
			map(result => {
				...
				return {
					targetModelIds: result.data.tankIds as string[],
	 ...
	 * @inheritDoc
	 */
	getAudits(): Observable<TankModelAudit[]> {
		...
		return this.apollo.query<any>(
			...
			map(result => {
				let audits: TankModelAudit[] = result.data.audits.map(audit => {
					return {
						entity: new TankModel(audit.entity),
	 ...
	 * @inheritDoc
	 */
	getAuditsByEntityId(id: string): Observable<TankModelAudit[]> {
		...
		return this.apollo.query<any>(
			{
				query: gql`
					query GetAuditsByEntityId($tankId: ID!) {
						audits: tankAuditsByEntityId(tankId: $tankId) {
							entity {
								...TankProperties
							}
							timestamp
