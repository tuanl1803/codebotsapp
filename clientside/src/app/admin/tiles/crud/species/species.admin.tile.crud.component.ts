...
import {getFishCollectionModels} from '../../../../models/fish/fish.model.selector';
...
@Component({
	selector: 'cb-admin-species-tile',
	templateUrl: './species.admin.tile.crud.component.html',
	styleUrls: [
		'./species.admin.tile.crud.component.scss',
...
export class AdminSpeciesTileCrudComponent implements OnInit {
	/**
	 ...
	 */
	@Input()
	collectionId: string = '288a26ed-f5ab-4104-9a3f-47a6dabd11f0';
	...
	onItemActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === ItemActionEnum.View) {
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'species', 'view', event.payload.model.id]));
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'species', 'edit', event.payload.model.id]));
	...
	onSwitchEdit() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'species', 'edit', this.targetModel.id]));
	...
	onCreateClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'species', 'create']));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			let afterwardActions: NgRxAction[] = [
				new routingAction.NavigateRoutingAction(['admin', 'entities', 'species'])
			];
	...
	onCancelClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'species']));
