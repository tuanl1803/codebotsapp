...
import {getSpeciesCollectionModels} from '../../../../models/species/species.model.selector';
import {TankModel} from '../../../../models/tank/tank.model';
import * as tankModelAction from '../../../../models/tank/tank.model.action';
import {getTankCollectionModels} from '../../../../models/tank/tank.model.selector';
...
@Component({
	selector: 'cb-admin-fish-tile',
	templateUrl: './fish.admin.tile.crud.component.html',
	styleUrls: [
		'./fish.admin.tile.crud.component.scss',
...
export class AdminFishTileCrudComponent implements OnInit {
	/**
	 ...
	 */
	@Input()
	collectionId: string = 'd3790bf2-11cf-488e-a365-80ba79def318';
	...
	constructor(
		...
		this.store.select(getCountFishModels).subscribe(
	...
	onItemActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === ItemActionEnum.View) {
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'fish', 'view', event.payload.model.id]));
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'fish', 'edit', event.payload.model.id]));
	...
	onSwitchEdit() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'fish', 'edit', this.targetModel.id]));
	...
	onCreateClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'fish', 'create']));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			let afterwardActions: NgRxAction[] = [
				new routingAction.NavigateRoutingAction(['admin', 'entities', 'fish'])
			];
	...
	onCancelClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'entities', 'fish']));
	...
	private prepareReferenceCollections() {
		...
		this.addSearchFunction(this.modelRelations.species, speciesModelAction.FetchSpeciesModelsWithQuery);
