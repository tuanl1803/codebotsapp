...
import * as modelAction from '../../../../models/admin/admin.model.action';
import {AdminModelState, AdminModelAudit} from '../../../../models/admin/admin.model.state';
import {AdminModel} from '../../../../models/admin/admin.model';
import {
	getAdminCollectionModels,
	getCountAdminModels,
	getAdminModelAuditsByEntityId,
	getAdminModelWithId
} from '../../../../models/admin/admin.model.selector';
...
@Component({
	selector: 'cb-admin-admin-tile',
	templateUrl: './admin.admin.tile.crud.component.html',
	styleUrls: [
		'./admin.admin.tile.crud.component.scss',
...
export class AdminAdminTileCrudComponent implements OnInit {
	/**
	 * The name of the fields which are used in the search box
	 */
	private searchFields: string[] = [
		...AdminModel.searchFields,
	...
	/**
	 * List of Admins that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 ...
	 */
	adminAudits$: Observable<AdminModelAudit[]>;
	 ...
	 */
	modelProperties: ModelProperty[] = AdminModel.getProps();
	 ...
	 */
	modelRelations: { [name: string]: ModelRelation } = AdminModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '2ce88f1a-db7c-4126-9ce4-d472030d48ed';
	...
	private get defaultExpands(): Expand[] {
		...
		let expands: Expand[] =  Object.entries(AdminModel.getRelations()).map(
			([key, entry]): Expand => {
				return {
					name: key,
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.adminsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	constructor(
		private readonly store: Store<{ model: AdminModelState }>,
		...
		this.admins = this.store.select(getAdminCollectionModels, this.collectionId);

		this.store.select(getCountAdminModels).subscribe(
			(count) => this.adminsCount = count
		);
	...
	ngOnInit() {
		...
		this.store.dispatch(new modelAction.InitialiseAdminCollectionState({
		...
		if (this.routerState.urls.includes('view') || this.routerState.urls.includes('edit')) {
			...
			const stateConfig: PassableStateConfig<AdminModel> = {
			...
			this.store.dispatch(new modelAction.FetchAdminModel(stateConfig));
			this.store.select(getAdminModelWithId, modelId).subscribe(model => {
			...
			this.prepareReferenceCollections();
			this.createReactiveForm();
			this.targetModel = new AdminModel();
			...
			this.store.dispatch(new modelAction.FetchAdminModelsWithQuery({
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CountAdminModels()
				]
			));
		}

		if (this.routerState.params.id) {
			this.adminAudits$ = this.store.select(getAdminModelAuditsByEntityId, this.routerState.params.id);
		}
	...
	onItemActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === ItemActionEnum.View) {
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'users', 'admin', 'view', event.payload.model.id]));
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'users', 'admin', 'edit', event.payload.model.id]));
			...
			if (confirm('Would you like to delete this model')) {
				...
				this.store.dispatch(new modelAction.DeleteAdminModel({
	...
	onMultipleItemsActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === MultipleItemActionEnum.Delete) {
			if (confirm('Would you like to delete this model')) {
				...
				this.store.dispatch(new modelAction.DeleteAdminModels({
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.FetchAdminModelAuditsByEntityId({
	...
	onSwitchEdit() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'users', 'admin', 'edit', this.targetModel.id]));
	...
	onCreateClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'users', 'admin', 'create']));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<AdminModel> = {
			...
			let afterwardActions: NgRxAction[] = [
				new routingAction.NavigateRoutingAction(['admin', 'users', 'admin'])
			];
			...
			this.store.dispatch(new modelAction.CreateAdminModel(
				stateConfig,
			...
			this.store.dispatch(new modelAction.UpdateAdminModel(
				stateConfig,
	...
	onCancelClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['admin', 'users', 'admin']));
	...
	onCollectionSort($event: OrderBy[]) {
		...
		this.store.dispatch(new modelAction.FetchAdminModelsWithQuery(stateConfig));
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = AdminModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.adminsCount - 1) / this.pageSize);
