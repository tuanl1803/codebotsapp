...
import {initialTankCollectionState} from './tank.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTankAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TankModelActionTypes.CREATE_TANK_OK:
				const createdModel = (action as modelAction.CreateTankModelOK).stateConfig.targetModel;
				...
				TankModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TankModelActionTypes.CREATE_TANK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TankModelActionTypes.CREATE_ALL_TANK_OK:
				const createdModels = (action as modelAction.CreateAllTankModelOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CreateAllTankModelOK).stateConfig.collectionId;
		
				// % protected region % [Add any additional CreateModelOK logic before main logic here] off begin
				// % protected region % [Add any additional CreateModelOK logic before main logic here] end
				
				createdModels.forEach((m) => {
					TankModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TankModelActionTypes.CREATE_ALL_TANK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TankModelActionTypes.DELETE_TANK_OK:
				const deletedModelId = (action as modelAction.DeleteTankModelOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.UpdateTankModelOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TankModelActionTypes.DELETE_ALL_TANK_OK:
				const deletedModelIds = (action as modelAction.DeleteTankModelOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.UpdateTankModelOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TankModelActionTypes.DELETE_TANK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TankModelActionTypes.UPDATE_TANK_OK:
				const updatedModel = (action as modelAction.UpdateTankModelOK).stateConfig.targetModel;
				...
				TankModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TankModelActionTypes.UPDATE_TANK_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TankModelActionTypes.UPDATE_ALL_TANK_OK:
				const updatedModels = (action as modelAction.UpdateAllTankModelOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.UpdateAllTankModelOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TankModelActionTypes.UPDATE_ALL_TANK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TankModelActionTypes.FETCH_TANK_OK:
				let fetchedModel = (action as modelAction.FetchTankModelOK).stateConfig.targetModel;
				...
				TankModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TankModelActionTypes.FETCH_ALL_TANK_OK:
				const totalCountAfterFetchAll = (action as modelAction.FetchAllTankModelsOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.FetchAllTankModelsOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.FetchAllTankModelsOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.FetchAllTankModelsOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TankModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TankModelActionTypes.FETCH_TANK_AUDIT_OK:
				const audits = (action as modelAction.FetchTankModelAuditOK).audits;
				...
				break;
			case modelAction.TankModelActionTypes.FETCH_TANK_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.FetchTankModelAuditsByEntityIdOK).audits;
				...
				break;
			case modelAction.TankModelActionTypes.FETCH_TANK_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.FetchTankModelsWithQueryOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.FetchTankModelsWithQueryOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.FetchTankModelsWithQueryOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.FetchTankModelsWithQueryOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TankModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TankModelActionTypes.FETCH_LAST_TANK_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.FetchLastTankModelsWithQueryOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.FetchLastTankModelsWithQueryOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.FetchLastTankModelsWithQueryOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.FetchLastTankModelsWithQueryOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TankModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TankModelActionTypes.COUNT_TANKS_OK:
				const totalCountAfterCount = (action as modelAction.CountTankModelsOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TankModelActionTypes.INITIALISE_TANK_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InitialiseTankCollectionState).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InitialiseTankCollectionState).stateConfig.queryParams;
