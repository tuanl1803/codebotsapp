...
/**
 * Effect class used for side effects related to Fish model.
 */
@Injectable()
export class FishEffect {
	...
	constructor(
		...
		protected readonly service: FishService,
	...
	countModels = this.action$.pipe(
		...
		ofType<modelAction.CountFishModels>(modelAction.FishModelActionTypes.COUNT_FISHS),
		...
		concatMap((action) => {
			...
			return this.service.count().pipe(
				...
				map(stateConfig => new modelAction.CountFishModelsOK(
					{
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.CountFishModelsFail(
							err
						)
					];
				}),
	...
	fetchedModel = this.action$.pipe(
		...
		ofType<modelAction.FetchFishModel>(modelAction.FishModelActionTypes.FETCH_FISH),
		...
		concatMap((action) => {
			...
			return this.service.get(modelId, queryParams).pipe(
				...
				map(stateConfig => new modelAction.FetchFishModelOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.FetchFishModelFail(
							modelId,
							err
						)
					];
	...
	fetchedModels = this.action$.pipe(
		...
		ofType<modelAction.FetchAllFishModels>(modelAction.FishModelActionTypes.FETCH_ALL_FISH),
		...
		concatMap((action) => {
			...
			return this.service.getAll().pipe(
				...
				map(stateConfig => new modelAction.FetchAllFishModelsOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.FetchAllFishModelsFail(
							err
						)
					];
				}),
	...
	fetchModelWithQuery = this.action$.pipe(
		...
		ofType<modelAction.FetchFishModelsWithQuery>(modelAction.FishModelActionTypes.FETCH_FISH_WITH_QUERY),
		...
		concatMap((action) => {
			...
			return this.service.getWithQuery(queryParams).pipe(
				...
				map(stateConfig => new modelAction.FetchFishModelsWithQueryOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.FetchFishModelsWithQueryFail(
							err
						)
					];
				}),
	...
	fetchLastModelWithQuery = this.action$.pipe(
		...
		ofType<modelAction.FetchLastFishModelsWithQuery>(modelAction.FishModelActionTypes.FETCH_LAST_FISH_WITH_QUERY),
		...
		concatMap((action) => {
			...
			return this.service.getLastWithQuery(queryParams).pipe(
				...
				map(stateConfig => new modelAction.FetchLastFishModelsWithQueryOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.FetchLastFishModelsWithQueryFail(
							err
						)
					];
				}),
	...
	createdModel = this.action$.pipe(
		...
		ofType<modelAction.CreateFishModel>(modelAction.FishModelActionTypes.CREATE_FISH),
		...
		concatMap((action) => {
			const modelToBeCreated = action.stateConfig.targetModel as FishModel;
			const collectionId = action.stateConfig.collectionId;
			const queryParams = action.stateConfig.queryParams;
			...
			return this.service.create(modelToBeCreated, queryParams).pipe(
				...
				map(stateConfig => new modelAction.CreateFishModelOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.CreateFishModelFail(
							modelToBeCreated,
							err
						)
					];
	...
	createAllModels = this.action$.pipe(
		...
		ofType<modelAction.CreateAllFishModel>(modelAction.FishModelActionTypes.CREATE_ALL_FISH),

		// % protected region % [Add any additional operations before using the service for createdModel here] off begin
		// % protected region % [Add any additional operations before using the service for createdModel here] end

		concatMap((action) => {
			const modelsToCreate = action.stateConfig.targetModels as FishModel[];
			const collectionId = action.stateConfig.collectionId;
			const queryParams = action.stateConfig.queryParams;
			...
			return this.service.createAll(modelsToCreate, queryParams).pipe(
				...
				map(stateConfig => new modelAction.CreateAllFishModelOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.CreateAllFishModelFail(
							modelsToCreate,
							err
						)
					];
	...
	updatedModel = this.action$.pipe(
		...
		ofType<modelAction.UpdateFishModel>(modelAction.FishModelActionTypes.UPDATE_FISH),
		...
		concatMap((action) => {
			const modelToBeUpdated = action.stateConfig.targetModel as FishModel;
			...
			return this.service.update(new FishModel({
				...modelToBeUpdated,
				...updates
			}), queryParams).pipe(
				...
				map(stateConfig => new modelAction.UpdateFishModelOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.UpdateFishModelFail(
							modelToBeUpdated,
							err
						)
					];
	...
	updateAllModels = this.action$.pipe(
		...
		ofType<modelAction.UpdateFishModel>(modelAction.FishModelActionTypes.UPDATE_ALL_FISH),
		...
		concatMap((action) => {
			const modelsToUpdate = action.stateConfig.targetModels as FishModel[];
			const updates = action.stateConfig.updates;
			const updatedModels = _.zipWith(modelsToUpdate, updates, (targetModel, update) => new FishModel({
				...targetModel,
				...update
			}));
			const queryParams = action.stateConfig.queryParams;
			...
			return this.service.updateAll(updatedModels, queryParams).pipe(
				...
				map(stateConfig => new modelAction.UpdateAllFishModelOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.UpdateAllFishModelFail(
							modelsToUpdate,
							err
						)
					];
	...
	deletedModel = this.action$.pipe(
		...
		ofType<modelAction.DeleteFishModel>(modelAction.FishModelActionTypes.DELETE_FISH),
		...
		concatMap((action) => {
			...
			return this.service.delete(modelIdToBeDeleted).pipe(
				...
				map(stateConfig => new modelAction.DeleteFishModelOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.DeleteFishModelFail(
							modelIdToBeDeleted,
							err
						)
					];
	...
	deletedModels = this.action$.pipe(
		...
		ofType<modelAction.DeleteFishModels>(modelAction.FishModelActionTypes.DELETE_ALL_FISH),
		...
		concatMap((action) => {
			...
			return this.service.deleteAll(modelIdsToBeDeleted).pipe(
				...
				map(stateConfig => new modelAction.DeleteFishModelsOK(
					{
						...action.stateConfig,
						...stateConfig,
				...
				catchError((err) => {
					return [
						new modelAction.DeleteFishModelsFail(
							modelIdsToBeDeleted,
							err
						)
					];
	...
	fetchedAudits = this.action$.pipe(
		...
		ofType<modelAction.FetchFishModelAudit>(modelAction.FishModelActionTypes.FETCH_FISH_AUDIT),
		...
		concatMap((action) => {
			...
			return this.service.getAudits().pipe(
				...
				map(audits => new modelAction.FetchFishModelAuditOK(
					audits,
				...
				catchError((err) => {
					return [
						new modelAction.FetchFishModelAuditFail(
							err
						)
					];
				}),
	...
	fetchedAuditsByEntityId = this.action$.pipe(
		...
		ofType<modelAction.FetchFishModelAuditsByEntityId>(modelAction.FishModelActionTypes.FETCH_FISH_AUDITS_BY_ENTITY_ID),
		...
		concatMap((action) => {
			...
			return this.service.getAuditsByEntityId(entityId).pipe(
				...
				map(audits => new modelAction.FetchFishModelAuditsByEntityIdOK(
					audits,
				...
				catchError((err) => {
					return [
						new modelAction.FetchFishModelAuditsByEntityIdFail(
							err
						)
					];
				}),
