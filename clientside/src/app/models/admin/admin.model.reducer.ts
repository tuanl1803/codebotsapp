...
import {initialAdminCollectionState} from './admin.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseAdminAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.AdminModelActionTypes.CREATE_ADMIN_OK:
				const createdModel = (action as modelAction.CreateAdminModelOK).stateConfig.targetModel;
				...
				AdminModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.AdminModelActionTypes.CREATE_ADMIN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdminModelActionTypes.CREATE_ALL_ADMIN_OK:
				const createdModels = (action as modelAction.CreateAllAdminModelOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CreateAllAdminModelOK).stateConfig.collectionId;
		
				// % protected region % [Add any additional CreateModelOK logic before main logic here] off begin
				// % protected region % [Add any additional CreateModelOK logic before main logic here] end
				
				createdModels.forEach((m) => {
					AdminModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.AdminModelActionTypes.CREATE_ALL_ADMIN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdminModelActionTypes.DELETE_ADMIN_OK:
				const deletedModelId = (action as modelAction.DeleteAdminModelOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.UpdateAdminModelOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdminModelActionTypes.DELETE_ALL_ADMIN_OK:
				const deletedModelIds = (action as modelAction.DeleteAdminModelOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.UpdateAdminModelOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdminModelActionTypes.DELETE_ADMIN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdminModelActionTypes.UPDATE_ADMIN_OK:
				const updatedModel = (action as modelAction.UpdateAdminModelOK).stateConfig.targetModel;
				...
				AdminModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdminModelActionTypes.UPDATE_ADMIN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.AdminModelActionTypes.UPDATE_ALL_ADMIN_OK:
				const updatedModels = (action as modelAction.UpdateAllAdminModelOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.UpdateAllAdminModelOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdminModelActionTypes.UPDATE_ALL_ADMIN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdminModelActionTypes.FETCH_ADMIN_OK:
				let fetchedModel = (action as modelAction.FetchAdminModelOK).stateConfig.targetModel;
				...
				AdminModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdminModelActionTypes.FETCH_ALL_ADMIN_OK:
				const totalCountAfterFetchAll = (action as modelAction.FetchAllAdminModelsOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.FetchAllAdminModelsOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.FetchAllAdminModelsOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.FetchAllAdminModelsOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => AdminModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdminModelActionTypes.FETCH_ADMIN_AUDIT_OK:
				const audits = (action as modelAction.FetchAdminModelAuditOK).audits;
				...
				break;
			case modelAction.AdminModelActionTypes.FETCH_ADMIN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.FetchAdminModelAuditsByEntityIdOK).audits;
				...
				break;
			case modelAction.AdminModelActionTypes.FETCH_ADMIN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.FetchAdminModelsWithQueryOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.FetchAdminModelsWithQueryOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.FetchAdminModelsWithQueryOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.FetchAdminModelsWithQueryOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => AdminModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdminModelActionTypes.FETCH_LAST_ADMIN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.FetchLastAdminModelsWithQueryOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.FetchLastAdminModelsWithQueryOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.FetchLastAdminModelsWithQueryOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.FetchLastAdminModelsWithQueryOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => AdminModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdminModelActionTypes.COUNT_ADMINS_OK:
				const totalCountAfterCount = (action as modelAction.CountAdminModelsOK).stateConfig.totalCount;
				...
				break;
			case modelAction.AdminModelActionTypes.INITIALISE_ADMIN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InitialiseAdminCollectionState).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InitialiseAdminCollectionState).stateConfig.queryParams;
