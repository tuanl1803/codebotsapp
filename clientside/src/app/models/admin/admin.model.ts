...
export class AdminModel extends UserModel {
	...
	get valueToDisplay(): string {
		...
		return AdminModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = AdminModel.modelPropGroups;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		AdminModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}
	 ...
	 */
	static deepParse(data: string | { [K in keyof AdminModel]?: AdminModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new AdminModel(data);
		}
	...
	constructor(data?: string | { [K in keyof AdminModel]?: AdminModel[K] }) {
		super(data);

		if (data) {
			const json = typeof data === 'string' ? JSON.parse(data) as { [K in keyof AdminModel]?: AdminModel[K] } : data;
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof AdminModel)) {
			return {};
		}
