...
@Component({
	selector: 'cb-tank-tile-crud',
	templateUrl: './tank.tile.crud.component.html',
	styleUrls: [
		'./tank.tile.crud.component.scss',
...
export class TankTileCrudComponent implements OnInit {
	/**
	 ...
	 */
	@Input()
	collectionId: string = '10e8ab60-2541-4752-8907-bad5cdd54175';
	...
	ngOnInit() {
		...
		this.store.select(getTankCollectionCount, this.collectionId).subscribe(
	...
	onItemActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === ItemActionEnum.View) {
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['tank-crud', 'view', event.payload.model.id]));
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['tank-crud', 'edit', event.payload.model.id]));
	...
	onSwitchEdit() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['tank-crud', 'edit', this.targetModel.id]));
	...
	onCreateClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['tank-crud', 'create']));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			let afterwardActions: NgRxAction[] = [
				new routingAction.NavigateRoutingAction(['tank-crud'])
			];
	...
	onCancelClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['tank-crud']));
