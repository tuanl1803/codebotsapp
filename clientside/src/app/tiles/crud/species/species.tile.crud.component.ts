...
import {SpeciesModelState, SpeciesModelAudit} from '../../../models/species/species.model.state';
import {SpeciesModel} from '../../../models/species/species.model';
import {
	getSpeciesCollectionModels,
	getCountSpeciesModels,
	getSpeciesModelAuditsByEntityId,
...
@Component({
	selector: 'cb-species-tile-crud',
	templateUrl: './species.tile.crud.component.html',
	styleUrls: [
		'./species.tile.crud.component.scss',
...
export class SpeciesTileCrudComponent implements OnInit {
	/**
	...
	/**
	 * List of Speciess that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	speciess: Observable<SpeciesModel[]>;

	/**
	 * Entity audits to be fetched from the server.
	 */
	speciesAudits$: Observable<SpeciesModelAudit[]>;

	/**
	 * The total number of entities in the database.
	 */
	speciessCount: number = 0;
	 ...
	 */
	@Input()
	collectionId: string = '7ac21b6d-bba8-4331-8580-30ac1b6286cb';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.speciessCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	constructor(
		...
		this.speciess = this.store.select(getSpeciesCollectionModels, this.collectionId);
	...
	ngOnInit() {
		...
		this.store.select(getSpeciesCollectionCount, this.collectionId).subscribe(
			(count) => this.speciessCount = count
		);
		...
		if (this.routerState.urls.includes('view') || this.routerState.urls.includes('edit')) {
			...
			const stateConfig: PassableStateConfig<SpeciesModel> = {
			...
			this.store.dispatch(new modelAction.FetchSpeciesModel(stateConfig));
			this.store.select(getSpeciesModelWithId, modelId).subscribe(model => {
			...
			this.prepareReferenceCollections();
			this.createReactiveForm();
			this.targetModel = new SpeciesModel();
		...
		if (this.routerState.params.id) {
			this.speciesAudits$ = this.store.select(getSpeciesModelAuditsByEntityId, this.routerState.params.id);
		}
	...
	onItemActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === ItemActionEnum.View) {
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['species-crud', 'view', event.payload.model.id]));
			...
			this.store.dispatch(new routingAction.NavigateRoutingAction(['species-crud', 'edit', event.payload.model.id]));
			...
			if (confirm('Would you like to delete this model')) {
				...
				this.store.dispatch(new modelAction.DeleteSpeciesModel({
	...
	onMultipleItemsActionClicked(event: { actionName: string, payload?: any }) {
		...
		if (event.actionName === MultipleItemActionEnum.Delete) {
			if (confirm('Would you like to delete this model')) {
				...
				this.store.dispatch(new modelAction.DeleteSpeciesModels({
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.FetchSpeciesModelAuditsByEntityId({
	...
	onSwitchEdit() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['species-crud', 'edit', this.targetModel.id]));
	...
	onCreateClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['species-crud', 'create']));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			let afterwardActions: NgRxAction[] = [
				new routingAction.NavigateRoutingAction(['species-crud'])
			];
			...
			this.store.dispatch(new modelAction.CreateSpeciesModel(
				stateConfig,
			...
			this.store.dispatch(new modelAction.UpdateSpeciesModel(
				stateConfig,
	...
	onCancelClicked() {
		...
		this.store.dispatch(new routingAction.NavigateRoutingAction(['species-crud']));
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.speciessCount - 1) / this.pageSize);
	...
	private prepareReferenceCollections() {
		...
		this.modelRelations.fishSpecies.stateConfig = {
			pageIndex: 0,
			pageSize: this.pageSize,
			collectionId: this.collectionId
		} as PassableStateConfig<FishModel>;

		this.store.dispatch(new fishModelAction.InitialiseFishCollectionState(this.modelRelations.fishSpecies.stateConfig));
		this.modelRelations.fishSpecies.collection = this.store.select(getFishCollectionModels, this.collectionId);
		this.addSearchFunction(this.modelRelations.fishSpecies, getFishCollectionModels, fishModelAction.FetchFishModelsWithQuery);
