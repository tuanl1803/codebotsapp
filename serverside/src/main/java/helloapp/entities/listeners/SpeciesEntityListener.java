...
import helloapp.configs.security.auditing.services.SpeciesEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SpeciesEntity.
 */
@Slf4j
@Component
public class SpeciesEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SpeciesEntityReadAuditService auditService;

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	@Autowired
	public void init(PermissionEvaluator permissionEvaluator, SpeciesEntityReadAuditService auditService) {
		SpeciesEntityListener.permissionEvaluator = permissionEvaluator;
		SpeciesEntityListener.auditService = auditService;
	}

	@PrePersist
	public void beforePersist(SpeciesEntity entity) {
	...
	@PostPersist
	public void afterPersist(SpeciesEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SpeciesEntity entity) {
	...
	@PostRemove
	public void afterRemove(SpeciesEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SpeciesEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SpeciesEntity entity) {
	...
	@PostLoad
	public void afterLoad(SpeciesEntity entity) throws JsonProcessingException {
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Species is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SpeciesEntity entity) {
		entity.unsetFishSpecies();
	}
