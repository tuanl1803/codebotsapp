...
import helloapp.configs.security.auditing.services.FishnaticEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing FishnaticEntity.
 */
@Slf4j
@Component
public class FishnaticEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static FishnaticEntityReadAuditService auditService;

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	@Autowired
	public void init(PermissionEvaluator permissionEvaluator, FishnaticEntityReadAuditService auditService) {
		FishnaticEntityListener.permissionEvaluator = permissionEvaluator;
		FishnaticEntityListener.auditService = auditService;
	}

	@PrePersist
	public void beforePersist(FishnaticEntity entity) {
	...
	@PostPersist
	public void afterPersist(FishnaticEntity entity) {
	...
	@PreRemove
	public void beforeRemove(FishnaticEntity entity) {
	...
	@PostRemove
	public void afterRemove(FishnaticEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(FishnaticEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(FishnaticEntity entity) {
	...
	@PostLoad
	public void afterLoad(FishnaticEntity entity) throws JsonProcessingException {
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Fishnatic is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(FishnaticEntity entity) {
	}
