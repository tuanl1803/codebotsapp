...
import helloapp.configs.security.auditing.services.FishEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing FishEntity.
 */
@Slf4j
@Component
public class FishEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static FishEntityReadAuditService auditService;

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	@Autowired
	public void init(PermissionEvaluator permissionEvaluator, FishEntityReadAuditService auditService) {
		FishEntityListener.permissionEvaluator = permissionEvaluator;
		FishEntityListener.auditService = auditService;
	}

	@PrePersist
	public void beforePersist(FishEntity entity) {
	...
	@PostPersist
	public void afterPersist(FishEntity entity) {
	...
	@PreRemove
	public void beforeRemove(FishEntity entity) {
	...
	@PostRemove
	public void afterRemove(FishEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(FishEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(FishEntity entity) {
	...
	@PostLoad
	public void afterLoad(FishEntity entity) throws JsonProcessingException {
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Fish is not permitted.");
		}
	}
